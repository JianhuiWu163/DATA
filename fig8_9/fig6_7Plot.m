...
for H=0:0.01:2-0.01
    ...
    plot3(op(1,:),op(2,:),op(3,:),'.','color',[255/255,204/255,51/255],'MarkerSize',3);hold on;
end
...
%----------------------------------------------------------------------%
for i=1:n
    plot3(px50(i,1),py50(i,1),pz50(1,1),'ro','MarkerSize',1);
    plot3(px50(i,2:5:800),py50(i,2:5:800),pz50(1,2:5:800),'r:');
    plot3(px50(i,loop),py50(i,loop),pz50(1,loop),'ro','MarkerSize',1);
end
%----------------------------------------------------------------------%
plot3(Xpt(1,1),Ypt(1,1),2.1,'g>','LineWidth',1,'MarkerSize',1);
...
figure(3)
set(gcf,'Position',[100 100 150 110]);%图片大小
set(gca,'Position',[.22 .25 .7 .7]);%坐标轴所占比例
% set(gca,'Position',[.18 .15 .7 .8]);%坐标轴所占比例

hp1 = plot(xt,Xcp10,'r',xt,Xcp50,'b',xt,Xpt,'g');hold on;
axis([0 loop*step 0 150]);
% set(gca,'XTickMode','manual','Xtick',[0:loop*step/5:loop*step]);
% set(gca,'YTickMode','manual','Ytick',[0:30:150]);
set(gca,'xtick',[0:loop*step/5:loop*step],'ytick',[0:30:150]);
set(gca,'TickDir','in','FontName','Times New Roman','FontSize',7);

xlabel('t (s)','Fontname','times new Roman','Fontsize',8);
ylabel('x (m)','Fontname','times new Roman','Fontsize',8);
% set(gca,'Fontname','times new Roman','Fontsize',7);%坐标刻度字体大小


text('interpreter','latex','string',['$\overline q','$'],'FontSize',8,'position', [125,140]);
text('interpreter','latex','string',['$\overline q','$'],'FontSize',8,'position', [125,120]);

hleg1=legend(hp1(1:3),'    of 10 \alpha-agents','    of 50 \alpha-agents','{q_\gamma}');
hleg1.ItemTokenSize = [15,9];
set(hleg1,'position', [0.62,.74,.1,.1],'Fontname','times new Roman','fontsize',7);
legend boxoff;
box off;
hold off;
...
set(gca,'YTickMode','manual','Ytick',[0:30:150]);
xlabel('t (s)','Fontname','times new Roman','Fontsize',8);
ylabel('y (m)','Fontname','times new Roman','Fontsize',8);
set(gca,'Fontname','times new Roman','Fontsize',7);%坐标刻度字体大小
box off;

text('interpreter','latex','string',['$\overline q','$'],'FontSize',8,'position', [117,140]);
text('interpreter','latex','string',['$\overline q','$'],'FontSize',8,'position', [117,120]);

hleg1=legend(hp1(1:3),'    of 10 \alpha-agents','    of 50 \alpha-agents','{q_\gamma}');
hleg1.ItemTokenSize = [15,9];
set(hleg1,'position', [0.6,.74,.1,.1],'Fontname','times new Roman','fontsize',7);
legend boxoff;
...
set(gcf,'Position',[100 100 220 140]);%图片大小
set(gca,'Position',[.18 .22 .76 .7]);%坐标轴所占比例
...
xlabel('Simulation times','Fontname','times new Roman','Fontsize',9);
ylabel('Minimum distance (m)','Fontname','times new Roman','Fontsize',9);
set(gca,'Fontname','times new Roman','Fontsize',8);%坐标刻度字体大小
grid on;
box off;

hleg1=legend(hp1(1:3),'10 \alpha-agents','20 \alpha-agents','30 \alpha-agents');
hleg1.ItemTokenSize = [15,9];
set(hleg1,'position', [0.39,.755,.1,.1],'Fontname','times new Roman','fontsize',8);
legend boxoff;

ah=axes('position',get(gca,'position'),'visible','off');
hleg2=legend(ah,hp1(4:5),'40 \alpha-agents','50 \alpha-agents');
hleg2.ItemTokenSize = [15,9];
set(hleg2,'position', [0.72,.798,.1,.1],'Fontname','times new Roman','fontsize',8);
legend boxoff;
